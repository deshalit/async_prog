...
from loader import Loader, ParsedFiles
...
def parse_files(worker_no: int, files: FileNames, logger: logging.Logger) -> ParsedFiles:
    ...
    for file in files:
        ...
        if not content:
            logger.info(f'Worker {worker_no}: file "{file}" was ignored')
            delete_file(file)
        else:
    ...
    if portion:
        parsed_files.update(portion)
    logger.info(f'Worker {worker_no}: parsing is complete')
    return parsed_files
...
def mp_handler(worker_no: int, files: FileNames):
    ...
    loader = Loader(worker_no, logger, parsed_files)
